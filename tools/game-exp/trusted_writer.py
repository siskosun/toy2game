...
def resolve_trusted_selection_rehearsal(
    ...
    if (
        ...
        raise DomainError(
            "current Rehearsal identity is invalid",
            code="DOMAIN_PREREQUISITE_MISSING",
        )
    ...
    fields = {
        ...
        "main_sha": str(rehearsal.get("main_sha") or ""),
        "source_sha": str(rehearsal.get("source_sha") or ""),
        "integration_sha": str(rehearsal.get("integration_sha") or ""),
        "integration_tree_sha": str(rehearsal.get("integration_tree_sha") or ""),
        "rehearsal_ref": str(rehearsal.get("rehearsal_ref") or ""),
        "workflow_source_sha": str(rehearsal.get("workflow_source_sha") or ""),
        "run_id": str(rehearsal.get("github_run_id") or ""),
        "run_attempt": str(rehearsal.get("github_run_attempt") or ""),
        "policy_digest": str(rehearsal.get("policy_digest") or ""),
        "scope_digest": str(rehearsal.get("scope_digest") or ""),
    }
    checks = rehearsal.get("checks")
    if not isinstance(checks, list) or not checks:
        raise DomainError(
            "current Rehearsal trusted checks are missing",
            code="DOMAIN_PREREQUISITE_MISSING",
        )

    match = re.fullmatch(r"EXP-([1-9][0-9]*)", experiment_id)
    if not match:
        raise DomainError("current Rehearsal experiment id is invalid")
    issue = match.group(1)
    expected_ref = f"refs/tags/exp-rehearsal/{issue}/{rehearsal_id}"
    if fields["rehearsal_ref"] != expected_ref:
        raise DomainError(
            "current Rehearsal ref is not canonical",
            code="DOMAIN_REHEARSAL_CONFLICT",
        )
    ...
    if ref_object.get("type") != "tag" or not isinstance(ref_object.get("sha"), str):
        raise DomainError(
            "current Rehearsal anchor must be an annotated tag",
            code="DOMAIN_REHEARSAL_CONFLICT",
        )
    tag_data = github_json(
        repo,
    ...
    tag_object = tag_data.get("object") or {}
    if tag_object.get("type") != "commit" or tag_object.get("sha") != fields["integration_sha"]:
        raise DomainError(
            "current Rehearsal anchor target differs from integration_sha",
            code="DOMAIN_REHEARSAL_CONFLICT",
        )

    message = tag_data.get("message")
    if not isinstance(message, str):
        raise DomainError("current Rehearsal anchor message missing")
    metadata: dict[str, str] = {}
    for line in message.splitlines():
        if ": " in line:
            key, item = line.split(": ", 1)
            metadata[key.strip()] = item.strip()
    expected_metadata = {
        "game-exp-experiment": fields["experiment_id"],
        "game-exp-candidate-id": fields["candidate_id"],
        "game-exp-rehearsal-id": fields["rehearsal_id"],
        "game-exp-main-sha": fields["main_sha"],
        "game-exp-source-sha": fields["source_sha"],
        "game-exp-integration-sha": fields["integration_sha"],
        "game-exp-integration-tree-sha": fields["integration_tree_sha"],
        "game-exp-workflow-source-sha": fields["workflow_source_sha"],
        "game-exp-run-id": fields["run_id"],
        "game-exp-run-attempt": fields["run_attempt"],
        "game-exp-policy-digest": fields["policy_digest"],
        "game-exp-scope-digest": fields["scope_digest"],
    }
    for key, expected in expected_metadata.items():
        if metadata.get(key) != expected:
            raise DomainError(
                f"current Rehearsal anchor metadata mismatch for {key}",
                code="DOMAIN_REHEARSAL_CONFLICT",
            )

    commit_data = github_json(
        repo,
        f"/git/commits/{urllib.parse.quote(fields['integration_sha'], safe='')}",
    )
    tree = commit_data.get("tree") or {}
    parents = commit_data.get("parents")
    parent_shas = [
        parent.get("sha") for parent in parents
        if isinstance(parent, dict)
    ] if isinstance(parents, list) else []
    if (
        tree.get("sha") != fields["integration_tree_sha"]
        or parent_shas != [fields["main_sha"], fields["source_sha"]]
    ):
        raise DomainError(
            "current Rehearsal integration commit tree/parents are invalid",
            code="DOMAIN_REHEARSAL_CONFLICT",
        )

    main_ref = github_json(repo, "/git/ref/heads/main")
    current_main = (main_ref.get("object") or {}).get("sha")
    if current_main != fields["main_sha"]:
        raise DomainError(
            "current Rehearsal is stale because main advanced",
            code="DOMAIN_REHEARSAL_CONFLICT",
        )

    run_data = github_json(
        repo,
        f"/actions/runs/{urllib.parse.quote(fields['run_id'], safe='')}",
    )
    if (
        str(run_data.get("run_attempt")) != fields["run_attempt"]
        or run_data.get("event") != "workflow_dispatch"
        or run_data.get("path") != ".github/workflows/game-exp-rehearsal.yml"
        or run_data.get("head_sha") != fields["workflow_source_sha"]
        or run_data.get("status") != "completed"
        or run_data.get("conclusion") != "success"
    ):
        raise DomainError(
            "current Rehearsal is not backed by a completed successful trusted workflow",
            code="DOMAIN_REHEARSAL_CONFLICT",
        )
    ...
    return TrustedRehearsalContext(
        ...
        integration_sha=fields["integration_sha"],
        integration_tree_sha=fields["integration_tree_sha"],
        rehearsal_ref=fields["rehearsal_ref"],
